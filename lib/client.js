...
module.exports = function (params) {
  ...
  this._bus = new PulseAudioBus(
    connect(params || { socket: detect.parse(detect.detectSync()) }));
...
PulseAudioBus.prototype._onMessage = function(msg) {
  ...
  var msg     = JSON.parse(JSON.stringify(msg)),
      ...
      handler = (msg.type == msgType.methodReturn ||
                 msg.type == msgType.error)  ? this._onMethodReturnOrError :
                (msg.type == msgType.signal) ? this._onSignal
                                             : this._onMethodCall;
...
PulseAudioBus.prototype._onMethodReturnOrError = function (msg) {
...
PulseAudioBus.prototype._onSignal = function (msg) {
  this.signals.emit(this.mangle(msg), msg.body, msg.signature);
};


PulseAudioBus.prototype._onMethodCall = function (msg) {
