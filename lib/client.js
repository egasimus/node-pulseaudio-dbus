...
var PulseAudio = module.exports = function (params) {
  ...
  this._bus.signals.on('org.PulseAudio.Core1.NewPlaybackStream:/org/pulseaudio/core1', function () {
    console.log("FUCK YEAH", arguments);
  })
...
PulseAudioBus.prototype.mangle = function(path, iface, member) {
  ...
  if (typeof path === 'object') { // handle one argument case mangle(msg)
    return path.interface + '.' + path.member + ':' + path.path;
  } else {
